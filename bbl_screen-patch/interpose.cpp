...
eject:
    ...
    // Parse a time string like "1h 17m 32s" into seconds without using QString::split.
    static int parseTimeString(const QString &s) {
        int secs = 0;
        QString num;
        for (int i = 0; i < s.length(); i++) {
            ...
            if (c.isDigit()) {
                ...
                secs += num.toInt() * 3600; num.clear();
            } else if (c == 'm' && !num.isEmpty()) {
                secs += num.toInt() * 60; num.clear();
            } else if (c == 's' && !num.isEmpty()) {
                secs += num.toInt(); num.clear();
            } else {
                num.clear();
            }
        }
    ...
    // Split str on sep without using QString::split (avoids Qt 5.14 ABI mismatch).
    static QStringList splitOn(const QString &str, QChar sep) {
        QStringList result;
        int start = 0;
        for (int i = 0; i <= str.length(); i++) {
            if (i == str.length() || str.at(i) == sep) {
                QString part = str.mid(start, i - start).trimmed();
    ...
    Q_INVOKABLE QString parseGcodeMetadata(QString path) {
        QJsonObject result;
        result["thumbnail"] = QString("");
        ...
        QString bestThumbnail, currentThumbnail;
        int bestPixels = 0, currentPixels = 0;
        bool inThumbnail = false;
        bool foundGcodeContent = false;
        QStringList filamentColors, filamentTypes;
        char buf[4096];
        int n = 0;

        while (n++ < 2000 && fgets(buf, sizeof(buf), fp)) {
            foundGcodeContent = true;
            size_t buflen = strlen(buf);
            bool lineDone = (buflen == 0 || buf[buflen-1] == '\n' || feof(fp));
            QString line = QString::fromUtf8(buf).trimmed();
            // Drain remainder of long lines (e.g. 50KB change_filament_gcode) that fgets truncated
            if (!lineDone) {
                char tmp[4096];
                while (!lineDone && fgets(tmp, sizeof(tmp), fp)) {
            ...
            if (line.contains("total estimated time:")) {
                ...
                if (secs > 0) result["timeEstimate"] = secs;
                continue;
            }
            ...
            // All remaining fields use '=' as separator (OrcaSlicer / older Bambu format)
            int eq = line.indexOf('=');
            if (eq < 0) continue;
            QString val = line.mid(eq + 1).trimmed();

            if (line.contains("estimated printing time") && line.contains("normal mode")) {
                int secs = parseTimeString(val);
                result["timeEstimate"] = secs;
            } else if (line.contains("filament used [g]")) {
                result["weightEstimate"] = val.toDouble();
